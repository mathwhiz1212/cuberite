...
bool cWSSAnvil::SaveChunkToNBT(const cChunkCoords & a_Chunk, cFastNBTWriter & a_Writer)
{
	...
	cEvent EventDataSerialised;
	bool DataRetrivalSucceeded;

	m_World->QueueTask([&Serializer, &EventDataSerialised, &a_Chunk, &DataRetrivalSucceeded](cWorld & a_World)
		{
			DataRetrivalSucceeded = a_World.GetChunkData(a_Chunk.m_ChunkX, a_Chunk.m_ChunkZ, Serializer);
			EventDataSerialised.Set();
		}
	);
	EventDataSerialised.Wait();
