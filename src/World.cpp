...
void cWorld::GenerateRandomSpawn(int a_PrepareDistance)
{
	...
	for (
		int ChunkX = NegativePrepareDistance,
		ChunkBlockX = ChunkX * cChunkDef::Width;

		ChunkX <= PositivePrepareDistance;

		++ChunkX,
		ChunkBlockX += cChunkDef::Width
	)
	{
		for (
			int ChunkZ = NegativePrepareDistance,
			ChunkBlockZ = ChunkZ * cChunkDef::Width;

			ChunkZ <= PositivePrepareDistance;

			++ChunkZ,
			ChunkBlockZ += cChunkDef::Width
		)
		{
			auto Biome = GetBiomeAt(ChunkBlockX, ChunkBlockZ);
...
IUsedAGotoEatThat:
	...
	// Check 0, 0 first.
	double SpawnY = 0.0;
	if (CanSpawnAt(BlockX, SpawnY, BlockZ))
	{
		m_SpawnX = BlockX + 0.5;
		m_SpawnY = SpawnY;
		m_SpawnZ = BlockZ + 0.5;
	...
	for (int RadiusOffset = 1; RadiusOffset < (a_PrepareDistance * 2); ++RadiusOffset)
	{
		for (int SearchGridIndex = 0; SearchGridIndex < PerRadiSearchCount; ++SearchGridIndex)
		{
			const Vector3i PotentialSpawn = Vector3i(BlockX, 0, BlockZ) + (ChunkOffset[SearchGridIndex] * RadiusOffset);
	...
	int Height;
	VERIFY(TryGetHeight(static_cast<int>(m_SpawnX), static_cast<int>(m_SpawnZ), Height));
	m_SpawnY = static_cast<decltype(Height)>(Height);
